...
void slidePartition(unsigned int x, unsigned int y);
void *psuedoMaster(void *arg);
void allRoadsLeadToRoam();
int getPartitionIndex(unsigned int gpX, unsigned int gpY);
void cleanSlate();
//==================================================================================
...
//pthread_mutex_t mainGrid;

vector<pthread_mutex_t> travelerLocks;
vector<vector<pthread_mutex_t>> mainGrid;

//std::vector<ThreadInfo> TA;
...
void updateMessages(void)
{
	...
	sprintf(message[2], "move partitions w/ space");
...
void handleKeyboardEvent(unsigned char c, int x, int y)
{
	...
	// spacebar to move a partition
	case 32:

		slidePartition(partitionList[partCounter].blockList[0].row, partitionList[partCounter].blockList[0].col);
		ok = 1;
		break;

	//  switch to a different partition
	case 'm':
		if (partCounter >= partitionList.size() - 1)
		{
			partCounter = 0;
		}
		else
	...
	else
	{
		...
		case NORTH:
			if (travelerList[travelerIndex].segmentList[0].row + 1 < numRows)
			{
				...
				else if (grid[travelerList[travelerIndex].segmentList[0].row + 1][travelerList[travelerIndex].segmentList[0].col] == FREE_SQUARE)
				{
					return true;
				}
				else if (grid[travelerList[travelerIndex].segmentList[0].row + 1][travelerList[travelerIndex].segmentList[0].col] == TRAVELER)
				{
		...
		case WEST:
			if (travelerList[travelerIndex].segmentList[0].col + 1 < numCols)
			{
				...
				else if (grid[travelerList[travelerIndex].segmentList[0].row][travelerList[travelerIndex].segmentList[0].col + 1] == FREE_SQUARE)
				{
					return true;
				}
				else if (grid[travelerList[travelerIndex].segmentList[0].row][travelerList[travelerIndex].segmentList[0].col + 1] == TRAVELER)
				{
					if (travelerList[travelerIndex].moveCount <= maxStall)
					{
						travelerList[travelerIndex].wouldStall = true;
...
void *threadFunc(void *arg)
{
	...
	// metrinome is used to swap between which 2 directins come first.
	// I ran into a problem where the travelers would wait if the best move it could make was one of the second 2.
	//  I couldnt figure out why, so i just made this work around. Instead of running for 20 - 30 function calls just to move into a free space, 
	// 	the traveler wiull now wait a max of 1.
	bool metrinome = true;

	localData = (struct ThreadInfo *)arg;
	unsigned int travelerIndex = localData->threadIndex;


	// full stop signals to the threads thast the program is closing
	while (!fullStop)
	{
		// buffers the time. Makes the speed up/slow down useless
		for (int i = 0; i < 75; i++)
		{
			usleep(500);
			usleep(500);
		...
		pthread_mutex_lock(&travelerLocks[travelerIndex]);


		// 	has the thread exited fully?
		if (travelerList[localData->threadIndex].segmentList.size() > 0)
		{
			...
			if (!travelerList[travelerIndex].exiting)
			{
				if (metrinome)
				{
					...
					if (travelerList[travelerIndex].segmentList[0].row + 1 < numRows)
					{
						pthread_mutex_lock(&mainGrid[travelerList[travelerIndex].segmentList[0].row + 1][travelerList[travelerIndex].segmentList[0].col]);
						...
						pthread_mutex_unlock(&mainGrid[travelerList[travelerIndex].segmentList[0].row + 1][travelerList[travelerIndex].segmentList[0].col]);
					}
					if (travelerList[travelerIndex].segmentList[0].col + 1 < numCols)
					{
						pthread_mutex_lock(&mainGrid[travelerList[travelerIndex].segmentList[0].row][travelerList[travelerIndex].segmentList[0].col + 1]);
						...
						pthread_mutex_unlock(&mainGrid[travelerList[travelerIndex].segmentList[0].row][travelerList[travelerIndex].segmentList[0].col + 1]);
					}
				...
				// 	if there is a best move, it will check to see if it needs to wait for another traveller to move on from it.
				// 	otherwise, if it has waited for a certain length of time, it will instead move randomly, since it is assumed 
				// 	that the traveler obstructing it is either it's own tail, or a dead traveler.
				//
				// 	if it moves randomly, it will continue to move in a straight line until it encounters a better move to make.
				...
				{	
					if (travelerList[travelerIndex].stall == false)
					{
						...
						case NORTH:
							...
							if (safeMove(travelerIndex, bestMove))
							{
							...
							{
								pthread_mutex_unlock(&mainGrid[travelerList[travelerIndex].segmentList[0].row][travelerList[travelerIndex].segmentList[0].col]);
							}
							break;
					...
					else
					{
						...
						if (travelerList[travelerIndex].moveCount > maxStall)
						{
							if (safeMove(travelerIndex, NORTH))
							{
			...
			pthread_mutex_unlock(&travelerLocks[travelerIndex]);
		}
		else
		{	
			// the thread is finished exiting on the grid, and shuts down.
			pthread_mutex_unlock(&travelerLocks[travelerIndex]);
...
// thread that launches other threads
void *psuedoMaster(void *arg)
{
	threadCount++;
	ThreadInfo TA[numTravelers];

	for (unsigned int i = 0; i < numTravelers; i++)
	{
		TA[i].threadIndex = i;
		if (pthread_create(&TA[i].threadID, NULL, threadFunc, (void *)&TA[i]) != 0)
		{
			cout << " traveler thread failed to be made" << endl;
			exit(0);
		}
	}
...
// when given a coordinate pair, it will determine if that point is a partition, then which partition it belongs to
int getPartitionIndex(unsigned int gpX, unsigned int gpY)
{

	SquareType tile = grid[gpX][gpY];

	if (tile == VERTICAL_PARTITION || tile == HORIZONTAL_PARTITION)
	{
		...
		for (unsigned int i = 0; i < partitionList.size(); i++)
		{
			...
			{
				if (partitionList[i].blockList[j].col == gpY && partitionList[i].blockList[j].row == gpX)
				{
					return i;
				}
			}
...
// slides a partition over by one tile, then recalculates the pathfinding for the map.
void slidePartition(unsigned int x, unsigned int y)
{

	int partitionIndex = getPartitionIndex(x, y);
	...
	{
		if (partitionList[partitionIndex].goingUp)
		{
			

			if (partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].row + 1 == numRows || grid[partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].row + 1][y] != FREE_SQUARE)
			{
			

				partitionList[partitionIndex].goingUp = false;
			}
			else
			{
				pthread_mutex_lock(&mainGrid[partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].row + 1][y]);

				

				grid[partitionList[partitionIndex].blockList[0].row][y] = FREE_SQUARE;
				gridPath[partitionList[partitionIndex].blockList[0].row][y] = 9999;
				grid[partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].row + 1][y] = VERTICAL_PARTITION;
				gridPath[partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].row + 1][y] = -1;

				for (unsigned int i = 0; i < partitionList[partitionIndex].blockList.size(); i++)
				{
					partitionList[partitionIndex].blockList[i].row++;
				}

				cleanSlate();
				allRoadsLeadToRoam();
				pthread_mutex_unlock(&mainGrid[partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].row][y]);
			}

			
		}
		else if (!partitionList[partitionIndex].goingUp)
		{
			

			if (partitionList[partitionIndex].blockList[0].row == 0 || grid[partitionList[partitionIndex].blockList[0].row - 1][y] != FREE_SQUARE)
			{
				

				partitionList[partitionIndex].goingUp = true;
			}
			else
			{
				pthread_mutex_lock(&mainGrid[partitionList[partitionIndex].blockList[0].row - 1][y]);
				

				grid[partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].row][y] = FREE_SQUARE;
				gridPath[partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].row][y] = 9999;
				grid[partitionList[partitionIndex].blockList[0].row - 1][y] = VERTICAL_PARTITION;
				gridPath[partitionList[partitionIndex].blockList[0].row - 1][y] = -1;
				...
				cleanSlate();
				allRoadsLeadToRoam();
				pthread_mutex_unlock(&mainGrid[partitionList[partitionIndex].blockList[0].row][y]);
			}

			
		}
	}
	else if (!partitionList[partitionIndex].isVertical)
	{

		if (partitionList[partitionIndex].goingUp)
		{
			
			if (partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].col + 1 == numCols || grid[x][partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].col + 1] != FREE_SQUARE)
			{
			...
			{
				pthread_mutex_lock(&mainGrid[x][partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].col + 1]);
				

				grid[x][partitionList[partitionIndex].blockList[0].col] = FREE_SQUARE;
				gridPath[x][partitionList[partitionIndex].blockList[0].col] = 9999;
				grid[x][partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].col + 1] = HORIZONTAL_PARTITION;
				gridPath[x][partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].col + 1] = -1;

				for (unsigned int i = 0; i < partitionList[partitionIndex].blockList.size(); i++)
				{
					partitionList[partitionIndex].blockList[i].col++;
				}

				cleanSlate();
				allRoadsLeadToRoam();
				pthread_mutex_unlock(&mainGrid[x][partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].col]);
			}
		...
		else if (!partitionList[partitionIndex].goingUp)
		{
			...
			if (partitionList[partitionIndex].blockList[0].col == 0 || grid[x][partitionList[partitionIndex].blockList[0].col - 1] != FREE_SQUARE)
			{
			...
			{
				pthread_mutex_lock(&mainGrid[x][partitionList[partitionIndex].blockList[0].col - 1]);
				

				grid[x][partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].col] = FREE_SQUARE;
				gridPath[x][partitionList[partitionIndex].blockList[partitionList[partitionIndex].blockList.size() - 1].col] = 9999;
				grid[x][partitionList[partitionIndex].blockList[0].col - 1] = HORIZONTAL_PARTITION;
				gridPath[x][partitionList[partitionIndex].blockList[0].col - 1] = -1;
				...
				cleanSlate();
				allRoadsLeadToRoam();
				pthread_mutex_unlock(&mainGrid[x][partitionList[partitionIndex].blockList[0].col]);
			}
...
// clears the pathfinding of values, used when a partition moves
void cleanSlate()
{
	for (unsigned int i = 0; i < numRows; i++)
	{
...
void initializeApplication(void)
{
	...
	for (unsigned int i = 0; i < numRows; i++)
	{
		...
		mainGrid.push_back(vector<pthread_mutex_t>());
		...
		for (unsigned int j = 0; j < numCols; j++)
		{
			...
			mainGrid[i].push_back(pthread_mutex_t());
		}
	}
	...
	for (unsigned int k = 0; k < numTravelers; k++)
	{
		...
		travelerLocks.push_back(travelerLock);

		travelerList.push_back(traveler);
	}
	allRoadsLeadToRoam();

	if (pthread_create(&psuedoMas, NULL, psuedoMaster, NULL) != 0)
	{
		cout << " distributor thread failed ot be made" << endl;
		exit(0);
	}
