...
{
	threadCount++;
	ThreadInfo *localData;
	localData = (ThreadInfo *)arg;
	unsigned int travelerIndex = localData->threadIndex;

	// full stop signals to the threads thast the program is closing
	// 	also checks if the thread has exited fully
	while (!fullStop && (travelerList[localData->threadIndex].segmentList.size() > 0))
	{
		...
		if (travelerList[travelerIndex].exiting != true)
		{
			...
			if (bestMoveVal != 99999)
			{
				if (travelerList[travelerIndex].stall == false &&
					(bestMove != travelerList[travelerIndex].segmentList[0].dir))
				{
